...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    *)
      ...
      echo "Usage: bash ops/scripts/local_up.sh [--force-restore]" >&2
      exit 1
      ;;
  esac
  shift
...
python3 "$ROOT_DIR/ops/scripts/setup_local_dev.py"
...
if [ -n "$LATEST_DUMP" ]; then
  if [ "$FORCE_RESTORE" -eq 1 ] || [ "$LATEST_DUMP" != "$PREVIOUS_DUMP" ]; then
    bash "$ROOT_DIR/ops/scripts/local_restore.sh" "$LATEST_DUMP"
  else
