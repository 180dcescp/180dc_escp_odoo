...
ENV_TEMPLATE_PATH = ROOT / ".env.local.example"
CONF_TEMPLATE_PATH = ROOT / "odoo.conf.local.template"
...
def render_local_env() -> str:
    return "\n".join(
        [
            ...
            "AUTHENTIK_OAUTH_BRIDGE_DISABLED=1",
            "",
        ]
    )
...
def main() -> int:
    ...
    env_example = ENV_TEMPLATE_PATH.read_text(encoding="utf-8")
    if "AUTHENTIK_OAUTH_BRIDGE_DISABLED=1" not in env_example:
        raise SystemExit("Unexpected .env.local.example contents.")
